...
def main_menu(cursor, conn, authenticated_email):
    ...
    column = ['property_id', 'street number', 'street name', 'city', 'state', 
               'room number', 'square foot', 'for rent', 'price', 'room amount', 'landlord_id']
...
>>>>>>> Stashed changes
                ...
                cursor.execute(f"SELECT * FROM properties ORDER BY {col} {asc}")
                output = cursor.fetchall()
                table = []
                for item in output:
                    table.append(list(item))
...
=======
            ...
            if filter == '2':
                ...
                if option == '1':
                    ...
                    cursor.execute(f'SELECT {col} FROM properties')
                    output = cursor.fetchall()
                    table = []
                    for item in output:
...
>>>>>>> Stashed changes
                    ...
                    cursor.execute(f"SELECT * FROM properties WHERE {col} = {asc}")
                    output = cursor.fetchall()
                    table = []
                    for item in output:
                        table.append(list(item))
                ...
                if option == '2':
                    ...
                    cursor.execute(f"SELECT * FROM properties WHERE {col} >= {low} AND {col} <= {high}")
                    output = cursor.fetchall()
                    table = []
                    for item in output:
                        table.append(list(item))
